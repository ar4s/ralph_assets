...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        db.add_column('ralph_assets_attachment', 'content_type',
        ...
        db.add_column('ralph_assets_attachment', 'object_id',
                      self.gf('django.db.models.fields.PositiveIntegerField')(default=None),
                      keep_default=False)

        if not db.dry_run:
            #  migrate all app with attachment to content type version
            for model in ('licence', 'asset', 'support'):
                content_type = orm['contenttypes.ContentType'].objects.get(
                    app_label='ralph_assets',
                    model=model,
                )
                sql = '''
                SELECT {model}_id, attachment_id
                FROM ralph_assets_{model}_attachments;'''.format(model=model)
                result = db.execute(sql)
                for object_id, attachment_id in result:
                    print object_id, attachment_id
                    attachment = orm.Attachment.objects.get(id=attachment_id)
    ...
    def backwards(self, orm):
        ...
        #  from content type to old structure
        if not db.dry_run:
            select_all = '''
            SELECT id, object_id, content_type_id
            FROM ralph_assets_attachment;
            '''
            for attachment_id, object_id, content_type_id in db.execute(select_all):
                content_type = orm['contenttypes.ContentType'].objects.get(
                    id=content_type_id,
                )
                sql = '''
                INSERT INTO ralph_assets_{model}_attachments ({model}_id, attachment_id)
                VALUES ('{model_id}', '{attachment_id}');'''.format(
        ...
        db.delete_column('ralph_assets_attachment', 'content_type_id')

        # Deleting field 'Attachment.object_id'
        db.delete_column('ralph_assets_attachment', 'object_id')
